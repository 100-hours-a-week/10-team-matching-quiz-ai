...
from app.vector_db.config import (
    RAG_TOP_K,
    SIM_THRESHOLD,
    RAG_DIVERSITY,
    Q_RAG_TOP_K,
...
def question_rag_retriever(
    ...
    try:
        ...
        if keyword:
            ...
            if keyword_phrases:
                k_vec = np.mean(
                    embed_texts(keyword_phrases, enrich_type="question"), axis=0
                )
        ...
        for _, doc_text, doc_vec in all_docs:
            ...
            k_sim = (
                safe_cosine_similarity(np.array(k_vec), np.array(doc_vec))
            ...
            if auto_keyword:
                keyword_list = set(
                    [kw.strip().lower() for kw in auto_keyword.split(",")]
                )
                keyword_bonus = (
                    sum(
                        1
...
def quiz_rag_retriever(
    questions: List[str], top_k: int = 1, sim_threshold: float = 0.7
) -> List[Dict[str, any]]:
    """
    ...
    try:
        ...
        for idx, question in enumerate(questions):
            logging.debug(
                f"질문 {idx+1}/{len(questions)} 처리 중: '{question[:50]}...'"
            )
            ...
            try:
                ...
                sorted_docs = sorted(
                    matched_docs, key=lambda x: x["similarity"], reverse=True
                )[:top_k]

                question_result = {"result": sorted_docs}

                results.append(question_result)

                logging.debug(
                    f"질문 '{question[:30]}...' 검색 완료: {len(sorted_docs)}개 문서 매칭"
                )
