...
import app.api.question_generator.question_generator_model as qg_model
import logging
...
async def initialize_llm_engine() -> None:
    ...
    try:
        ...
        if qg_model.llm:
            ...
            logger.error(
                "LLM 초기화 후 'qg_model.llm' (question_generator_model.llm)이 None입니다."
            )
...
async def cleanup_llm_engine() -> None:
    """LLM 엔진 리소스를 정리합니다."""
    if qg_model.llm and hasattr(qg_model.llm, "shutdown_background_loop"):
        try:
            logger.info("AsyncLLMEngine 백그라운드 루프 종료를 시도합니다...")
            qg_model.llm.shutdown_background_loop()
        ...
        logger.info(
            "AsyncLLMEngine (qg_model.llm)이 초기화되지 않았거나 'shutdown_background_loop' 메소드가 없어 "
            "종료 처리를 건너뜁니다."
        )
