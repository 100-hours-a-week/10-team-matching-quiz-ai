...
def generate_quiz_api(req: FollowupRequest):
    ...
    prompt += (
        "\n\n출력 지침:\n"
        "- 출력은 반드시 15~20개의 4지선다형 문제로 구성되어야 하며, 각각의 문제는 아래와 같은 필드를 포함해야 합니다:\n"
        "  · 난이도 (하|중|상)\n"
        "  · 문제 (자연어 문장)\n"
        "  · 선지 (4개의 보기, JSON 배열 형태)\n"
        "  · 정답 인덱스 (1~4 사이의 숫자)\n"
        "  · 해설 (간단한 설명)\n"
        "- 설명 문구, 프롬프트 내용, 안내 문장은 출력에 포함하지 말고 **문제 데이터만 연속적으로 출력**할 것.\n"
        "- JSON이 아닌 자유 포맷의 텍스트로 출력하되, 각 필드 간 줄바꿈은 유지할 것.\n"
        "--- END OF INSTRUCTION ---"
    )
